...
serve(async (req) => {
  ...
  try {
    ...
    // Find 'auth' in the path and get the next part
    // For URL like /functions/v1/auth/login, we want 'login'
    ...
    console.log('Auth index:', authIndex);
    ...
    switch (path) {
      ...
      case 'google-link':
        console.log('Calling handleLinkGoogleAccount');
        return await handleLinkGoogleAccount(supabaseAdmin, authHeader, data);
        
      case 'google-unlink':
        console.log('Calling handleUnlinkGoogleAccount');
        return await handleUnlinkGoogleAccount(supabaseAdmin, authHeader);
      ...
      default:
        console.error('Unknown auth route:', path);
        return new Response(
          JSON.stringify({ 
            error: `Unknown auth route: ${path}`,
            availableRoutes: [
              'register',
    ...
    console.error('=== ERROR IN AUTH FUNCTION ===');

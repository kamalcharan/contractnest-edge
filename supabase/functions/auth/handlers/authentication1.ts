// supabase/functions/auth/handlers/authentication.ts
...
export async function handleLogin(supabase: any, data: any) {
  ...
  try {
    ...
    console.log('Found', userTenantData?.length || 0, 'assigned tenants for user');
    
    // Get user's roles for each tenant
    ...
    const finalTenants = (userTenantData || []).map(ut => {
      const tenantRoles = userTenantRoles[ut.tenant_id] || [];
      const isOwner = ut.t_tenants.created_by === authData.user.id;
      const isTenantAdmin = isOwner || tenantRoles.includes('Owner') || tenantRoles.includes('Admin');

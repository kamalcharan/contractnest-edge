// supabase/functions/create-google/index.ts
...
if (!existingAuthMethod) {
  ...
  const { data: authMethodData, error: authMethodError } = await supabaseAdmin
    .from('t_user_auth_methods')
    .insert({
      user_id: user.id,
      auth_type: 'google',
  ...
  if (authMethodError) {
    ...
    console.error('Auth method error details:', {
      message: authMethodError.message,
      details: authMethodError.details,
      hint: authMethodError.hint,
      code: authMethodError.code
    })
    // Don't fail the whole operation, but log it
  } else {
    console.log('Auth method created successfully:', authMethodData)
  }
} else {

...
-- Purpose: Update existing 15 parents + insert 6 new parents + insert 47 sub-segments
...
-- (Safe to rerun — just resets values that should already be correct)
-- ============================================================================
UPDATE public.m_catalog_industries
SET level = 0, segment_type = 'segment'
WHERE parent_id IS NULL AND level = 0;

-- Also handle any rows where level/segment_type might be NULL from partial apply
UPDATE public.m_catalog_industries
SET level = 0, segment_type = 'segment'
WHERE parent_id IS NULL;

-- ============================================================================
-- STEP 2: Insert 6 new parent industries (ON CONFLICT skip)
-- ============================================================================
...
-- ============================================================================
-- STEP 3: Insert sub-segments (47 total) — ON CONFLICT skip duplicates
-- ============================================================================
...
-- Per spec: if a parent has no sub-segments, selection of parent alone is sufficient.


-- ============================================================================
-- ROLLBACK (commented out — run manually if needed, in reverse FK order)
-- ============================================================================

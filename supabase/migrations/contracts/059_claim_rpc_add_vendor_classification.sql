-- ═══════════════════════════════════════════════════════════════
-- Migration 059: claim_contract_by_cnak — add vendor classification
-- ═══════════════════════════════════════════════════════════════
-- Problem:  When a buyer claims a contract via CNAK, the seller
--           is created as a corporate contact in the buyer's
--           workspace but WITHOUT any classification. This means
--           the contact doesn't appear under the "Vendor" filter
--           and the contract detail page can't label it properly.
--
-- Fix:     1. Re-create claim_contract_by_cnak RPC to include
--             classifications = [{"classification_value":"vendor",
--             "classification_label":"Vendor"}] on INSERT.
--          2. Backfill existing contacts created via cnak_claim
--             that are missing the vendor classification.
--
-- Safety:  - Uses CREATE OR REPLACE (no downtime)
--          - Backfill only touches rows WHERE source = 'cnak_claim'
--            AND classifications is NULL or empty
...
-- ─────────────────────────────────────────────────────────────
-- PART 1: Backfill existing claimed contacts missing classification
-- ─────────────────────────────────────────────────────────────

UPDATE t_contacts
SET classifications = '[{"classification_value": "vendor", "classification_label": "Vendor"}]'::jsonb,
    updated_at = NOW()
WHERE source = 'cnak_claim'
  AND is_active = true
  AND (classifications IS NULL OR classifications = '[]'::jsonb);


-- ─────────────────────────────────────────────────────────────
-- PART 2: Re-create claim_contract_by_cnak with classification
-- ─────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- FIX (059): Now includes vendor classification
    -- ═══════════════════════════════════════════
...
GRANT EXECUTE ON FUNCTION claim_contract_by_cnak(TEXT, UUID, UUID) TO service_role;

COMMENT ON FUNCTION claim_contract_by_cnak IS 'Claim a contract by CNAK code. Creates seller as corporate contact with vendor classification in buyer workspace. Supports both manual-accept and auto-accept contracts.';

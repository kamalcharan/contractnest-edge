...
-- 1. Adds metadata JSONB column to t_contracts
-- 2. Updates create_contract_transaction to store metadata
-- 3. Updates update_contract_transaction to store/update metadata
-- 4. Updates get_contract_by_id to return metadata
--
-- Purpose: Enables save-as-draft wizard state persistence.
--          The wizard serializes its full state into metadata.wizard_state
--          so drafts can be resumed from the exact step where the user left off.
--
-- Dependencies:
...
COMMENT ON COLUMN t_contracts.metadata
  IS 'Flexible JSONB metadata. Used by wizard for draft state persistence: {wizard_state, wizard_step, wizard_contract_type}.';
...
COMMENT ON FUNCTION create_contract_transaction IS 'Creates a contract with blocks, vendors, history, access, computed_events, nomenclature, equipment_details, coverage_types, metadata, start_date, and allow_buyer_to_add_equipment in a single transaction';
    ...
    BEGIN
        v_response := jsonb_build_object(
            ...
            'data', jsonb_build_object(
                ...
                'metadata', COALESCE(v_current.metadata, '{}'::JSONB),
...
COMMENT ON FUNCTION update_contract_transaction IS 'Updates a contract with optimistic concurrency, audit trail, equipment_details, coverage_types, metadata, start_date, and allow_buyer_to_add_equipment';
...
COMMENT ON FUNCTION get_contract_by_id IS 'Returns full contract detail with blocks, vendors, attachments, history, equipment, nomenclature, metadata, start_date, equipment flag, and coverage_types';

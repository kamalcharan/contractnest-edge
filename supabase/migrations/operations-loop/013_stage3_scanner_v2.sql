-- ============================================================================
-- Migration: Stage 3 Appointments — 013 Scanner v2 (appointment requests)
-- ============================================================================
-- Purpose: adds STEP 2b to the Stage 0 runtime loop — service events at
-- due/overdue within p_appointment_lead_days (default 6) with no active
-- appointment get a 'requested' t_appointments row (idempotent via the
-- unique per-event index). MANUAL MODE: no JTD dispatch from appointments —
-- the owner works the kanban; VaNi-stage adds auto-chasing.
-- The function signature gains a parameter, so the old 4-arg overload is
-- DROPPED first (otherwise `SELECT run_contract_event_scanner()` from cron
-- would be ambiguous). The cron job itself needs no change.
-- Everything else is byte-identical to 004 (+ the marked STAGE 3 CHANGES).
-- Depends on: 004 (scanner), 012 (t_appointments)
-- Safe to re-run: Yes
...
CREATE OR REPLACE FUNCTION run_contract_event_scanner(
    ...
    p_appointment_lead_days      INT DEFAULT 6,   -- STAGE 3: request appointments this many days ahead
...
DECLARE
    ...
    c_appointments_requested INT := 0;   -- STAGE 3 CHANGE
...
BEGIN
    ...
    -- ═══════════════════════════════════════════
    -- STEP 2b (STAGE 3): service events near/past date with no appointment →
    -- create a 'requested' t_appointments row (manual mode: no JTD dispatch;
    -- the owner works the board — VaNi-stage adds auto-chasing)
    -- ═══════════════════════════════════════════
    FOR v_row IN
        ...
        WHERE e.is_active = true
          ...
          AND e.scheduled_date < date_trunc('day', now()) + make_interval(days => p_appointment_lead_days + 1)
          AND NOT EXISTS (
    ...
    LOOP
        ...
        EXCEPTION
            WHEN unique_violation THEN
                NULL; -- another run won the race — idempotency holding
            WHEN OTHERS THEN
                c_errors := c_errors + 1;
                IF COALESCE(array_length(v_error_samples, 1), 0) < 5 THEN
